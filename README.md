...
| **2025.12.04** | **董泽福** | ✅ 初步实现敌机的生成和移动
| **2025.12.05** | **宋博文** | ✅ 初步实现飞机的换皮肤，换地图功能
| **2025.12.05** | **董泽福** | ✅ 新增Enemy类封装敌机血量和爆炸逻辑。对飞机生成位置修改，使其完全在界面内
| **2025.12.06** | **宋博文** | ✅ 实现了音效功能，添加了菜单内和游戏内背景音乐
| **2025.12.10** | **宋博文** | ✅ 初步完成了我方战斗机和敌方战斗机的子弹发射功能以及碰撞功能，并添加了gameover函数及游戏结束音效
| **2025.12.12** | **宋博文** | ♻️ **核心架构重构 (Refactoring)**：<br>1. 引入 `BaseEntity` 基类，实现多态继承体系<br>2. 拆解 `HelloWorldScene`，将主角逻辑封装至 `Player` 类，敌机逻辑封装至 `Enemy` 类<br>3. 修复 LNK2019/LNK2001 链接错误及资源加载崩溃问题<br>🔧 **工程优化**：解决 VS2022 中文注释提交乱码问题 | **OOD 架构里程碑** |
| **2025.12.17** | **董泽福** | 初步添加boss并实现boss的移动与爆炸，修改实现了小敌机的子弹速度和其生成位置尽可能规避boss的位置。初步实现血条的使用。
| **2025.12.21** | **宋博文** | ✨ 游戏反馈与AI逻辑增强：完善Boss AI巡逻与血条逻辑，新增计分飘字特效、受伤反馈及爆炸音效，修复射击相关Bug | 游戏反馈与AI优化 |
| 2025.12.24 | 宋博文 | 🚀 核心机制扩展：构建道具掉落与拾取系统（回血/火力升级），实现主角多级武器成长机制（弹幕样式/伤害数值/音效随等级进阶）；重构敌机生态，新增精英怪（散射导弹/多皮肤随机），统一实现了敌机素材的标准化缩放与朝向适配逻辑。 | 道具系统与敌机多样性 |
